...
int OpenArm::add_motor(damiao_motor::MotorType motor_type, uint32_t send_can_id,
    ...
    // Create motor object
    motors_[motor_count_] =
        std::make_unique<damiao_motor::Motor>(motor_type, send_can_id, recv_can_id);
...
bool OpenArm::set_mode_all_rt(ControlMode mode, int timeout_us) {
    ...
    size_t written =
        write_param_all_rt(openarm::damiao_motor::RID::CTRL_MODE, mode_value, timeout_us);

    return written == motor_count_;
}

bool OpenArm::encode_mit_command(const damiao_motor::Motor& motor,
                                 const damiao_motor::MITParam& param, can_frame& frame) {
...
bool OpenArm::encode_posvel_command(const damiao_motor::Motor& motor,
...
bool OpenArm::encode_simple_command(const damiao_motor::Motor& motor, uint8_t cmd,
                                    can_frame& frame) {
...
void OpenArm::packet_to_frame(const damiao_motor::CANPacket& packet, can_frame& frame) {
...
void OpenArm::frame_to_data(const can_frame& frame, std::vector<uint8_t>& data) {
